...
if __name__ == "__main__":
    # Define network architecture parameters based on your data
    # MACCS keys are often 166 bits
    INPUT_SIZE_DRUG = 166
    # ProtBERT embeddings are often 1024 dimensions
    INPUT_SIZE_PROTEIN = 1024

    HIDDEN_SIZE = 32  # Number of neurons in the hidden layer (tunable)
    OUTPUT_SIZE = 1  # Single output: 0 (no bind) or 1 (binds)

    LEARNING_RATE = 0.5
    TRAINING_EPOCHS = 20000  # Can train much faster with numpy
    ...
    print(f"Input Drug Features: {INPUT_SIZE_DRUG}")
    print(f"Input Protein Features: {INPUT_SIZE_PROTEIN}")
    print(f"Total Input Features: {INPUT_SIZE_DRUG + INPUT_SIZE_PROTEIN}")
    ...
    nn.train(X_drug_data, X_protein_data, y_data, TRAINING_EPOCHS)

    # --- Test the Network ---
    print("\n--- Predictions after Training (on mock data) ---")
    predictions = nn.predict(X_drug_data, X_protein_data)

    for i in range(len(y_data)):
        pred_value = predictions[i][0]
        # Round the output to get a clear 0 or 1 prediction
        pred_class = 1.0 if pred_value > 0.5 else 0.0
        print(f"Sample {i}: Prediction: {pred_value:.4f} (Class: {pred_class}), Target: {y_data[i][0]}")