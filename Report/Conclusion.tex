...
It starts by preprocessing raw datasets and engineering features with industry-standard encodings, culminating in an iteratively developed deep neural network.
The pipeline was made accessible with a React-based interface through a FastAPI backend, providing a virtual screening interface designed for use in the early stages of drug discovery.
...
Second, the project demonstrates an iterative development methodology, showing incremental improvements and providing transparency on how architectural, training, and framework decisions impact model performance.
Third, training on a recent (November 2025) snapshot of BindingDB demonstrates that the pipeline works on current data rather than a frozen historical subset.
...
Iterating from a NumPy implementation (building the backpropagation function from scratch) to PyTorch provided insight into the mechanics that frameworks abstract away.
