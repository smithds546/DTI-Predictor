...
The data instances of drug molecules and target proteins are represented as fixed length numerical vectors which $\mathbf{d} \in \mathbb{R}^{m}$ denote the representation of a drug molecule and $\mathbf{p} \in \mathbb{R}^{n}$ denote the representation of a target protein.
...
Allowing independent processing preserved the features and semantic meaning for each drug and protein, and joint learning is enabled once combined.
...
Here is a tale describing the architecture of the prediction network:
  ...
  \midrule
  \textit{Architecture} \\
  \quad Hidden size          & DrugEncoder hidden 256→128, ProteinEncoder 1024→512→256→128, Predictor 256→256→128→64→1\\
...
The weight decay parameter was set to 0.0001, which helps to prevent overfitting by adding a penalty on large weights, encouraging the model to learn simpler patterns that generalise better to unseen data.
...
This figure is the model prediction of either binding (1) or non-binding (0).
