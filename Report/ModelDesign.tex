...
The prediction task is formulated as a supervised binary classification problem, where the goal is to predict a given drug and protein pairs binding interaction.
...
To obtain a binary interaction label, the predicted score $\hat{y}$ is compared to the decision threshold $\tau$ = 0.3, which is justified through data sensitivity analysis in Chapter~\ref{ch:-evaluation}:
\[
\hat{y}_{\text{class}} =
\begin{cases}
...
The learning objective is to minimise the discrepancy between the predicted interaction labels $\hat{y}$ and the true labels $y$ in the training dataset.

\section{Integration of Input Representations}
Drug to Target interaction prediction requires the combination of two different data styles, drugs being small chemical structures and proteins being amino acid sequences.
...
Drug molecules are represented using MACCS keys~\citep{durant2002reoptimization}, which encode the presence or absence of predefined chemical substructures as a fixed length binary vector of 167 dimensions.
Each SMILES string is parsed into an RDKit object, from which the MACCS fingerprint is generated.
Where a SMILES string cannot be successfully parsed, a zero vector is returned to preserve input dimensional consistency and prevent pipeline failure.
...
Each protein sequence is tokenised and passed through the frozen ProtBERT model to get per-token hidden states.
Mean pooling is then applied across the sequence dimension of the final hidden layer, aggregating variable-length protein sequences while preserving global contextual information learned by the transformer.
This produces a single 1024-dimensional embedding that summarises the entire sequence; this is the aggregation strategy used in the original ProTrans work~\citep{Elnaggar2020.07.12.199554}.
Because this process is computationally expensive, each embedding is cached locally using hash-based lookup, significantly reducing overhead during dataset re-processing when previously computed embeddings can be loaded directly from disk.
...
The drug encoder is a three-layer MLP (167$\rightarrow$256$\rightarrow$128) that projects the 167-bit MACCS fingerprint into the shared 128-dimensional latent space, allowing the model to learn which substructural features are most relevant for interaction prediction.
...
The full dual-branch architecture is defined as three PyTorch \texttt{nn.Module} classes: \texttt{DrugEncoder}, \texttt{ProteinEncoder}, and \texttt{InteractionPredictor}, inside the top-level \texttt{DTI\_DNN} module.
...
Stronger tapers underuse the regularisation budget in deeper layers, while weaker tapers produced less stable training.
The $0.67$ value kept regularisation without visible degradation during the iterative experiments described in Chapter~\ref{ch:-evaluation}.
...
Once both inputs have been individually encoded to have dimensionality of $128$ and shared to the latent space, they are concatenated into a single vector with dimensionality $256$ representing the drug-target pair.
