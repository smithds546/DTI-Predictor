...
By flagging promising candidates up front, the system is intended to reduce the sample size taken forward to wet-lab validation and, in turn, shorten the development cycles the pharmaceutical industry faces.
...
Drug-target interaction was investigated primarily through wet-lab experiments until the first \textit{in silico} approach, molecular docking against 3D structures, was introduced in 1982 \citep{Ru2025InSilicoDTI}.
...
The website was developed using the React framework, chosen for its modular architecture, rapid development workflow, and live rendering~\citep{aggarwal2018react}.
The interface was designed to be straightforward, easy to navigate, and to present only the information relevant to the workflow.
...
The project is restricted to publicly available datasets (e.g.\ BindingDB, ChEMBL), which may contain missing or biased entries introduced during curation.
The dataset may also be incomplete, omitting information that could otherwise improve the model's predictive performance.
...
The model was trained on a personal laptop, so the available compute budget is not representative of what industrial pharmaceutical research groups can deploy, widening the gap between this prototype and industry-scale systems.
...
The project assumes that users possess sufficient prior chemistry knowledge to interpret the input fields and results.
